			...
			{#each $concept.user as value, idx}
				{#if !on}
					<div class="w-[45px] h-[45px] my-1">
						<img
							style="border-color: {value.userId === $user?.id
								? 'white'
								: $colors[idx % $colors.length]};"
							class="shadow-md {isSpeaking ? 'p-2' : ''} transition-all border-4 rounded-full"
				...
				{:else}
					<div
						class="relative flex flex-col items-center justify-center w-[280px] h-[150px] my-2 border-2 dark:bg-white bg-black {value.userId ===
						$user?.id
							? 'dark:border-white'
					...
					>
						{#if null === null}
							<div
								style="background: {value.userId === $user?.id
									? $darkMode
										? '#111317'
										: 'white'
									: $colors[idx % $colors.length]} ; border-radius: 0.4rem;"
								class="flex flex-col items-center justify-center w-full h-full rounded-lg"
							>
								<img
									class=" w-[75px] h-[75px] transition-all {isSpeaking
										? 'p-4'
										: 'p-2'} border-4 {value.userId === $user?.id
										? 'dark:border-white'
						...
						{:else}
							<video
								...
								autoplay
								playsinline
								class="w-[280px] h-[150px] border-2 border-black"
								id="user-video-{value.userId}"
							>
								<track kind="captions" />
							</video>
						{/if}
