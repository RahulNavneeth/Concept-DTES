...
<div
	class="absolute shadow-md top-0 z-[1000] bg-white w-full h-[45px] border-b-[1px] dark:border-white border-black flex flex-row py-2 items-center justify-between font-black font-primary text-xl dark:bg-darkblack dark:text-white"
>
	...
	<div class="w-1/6 flex flex-col items-end">
		{#if $concept && $page.route.id === '/concept/[id]'}
			<div class="flex flex-row items-center justify-center">
				{#each $concept.user.slice(0, 2) as value}
					<img
						class="w-[35px] h-[35px] -ml-2 rounded-full"
						src={value.user.image}
						alt="profileImg"
					/>
				{/each}
				<button
					class="w-[35px] h-[35px] -ml-2 bg-white border-2 border-darkblack rounded-full dark:bg-[#2B3241] dark:text-white text-sm flex flex-col items-center justify-center pb-1"
					>...</button
				>
			</div>
		{/if}
	...
	<div class="w-1/6 flex flex-row items-center relative justify-end px-2">
		{#if $user}
			...
			>
				<img class="w-[30px] h-full rounded-full" src={$user.image} alt="profileImg" />
			</button>
		{:else}
			<a class="mx-2 underline" href="/login"> LOGIN </a>
		{/if}
	</div>
</div>
